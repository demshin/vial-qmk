...
Программа доступна для ОС Windows и Linux, скачать ее можно по [ссылке](https://github.com/ergohaven/qmk-hid-host/releases/tag/latest). Это форк, в котором изменены настройки по умолчанию с той целью, чтобы для клавиатур Ergohaven не приходилось редактировать файл настроек. Автор [оригинальной программы](https://github.com/zzeneg/qmk-hid-host) zzeneg, за что ему большое спасибо.
